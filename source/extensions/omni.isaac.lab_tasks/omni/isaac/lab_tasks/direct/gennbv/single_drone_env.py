...
from .single_drone_env_cfg import GENNBVEnvCfg
import random
import glob
import os
import numpy as np
...
class GENNBVEnv(DirectRLEnv):
    cfg: GENNBVEnvCfg

    def __init__(self, cfg: GENNBVEnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def _setup_scene(self):
        ...
        #self.gt_pointclouds = np.zeros((self.cfg.num_envs, 10000, 3))
                ...
                if line:  # Ignore empty lines
                    *path_parts, _ = line.rsplit(maxsplit=1)  # Split by spaces, last part is the value
                    ...
                    if hollow_occ_path.endswith("hollow_occ.npy"):
                        ...
                        #pointcloud_path = os.path.join(directory.replace("/home/Dataset/houes3k_env20//preprocess", "/home/hat/Documents/Dataset/test_pointcloud"), os.path.basename(usd_files[0]).replace(".usd", ".ply"))
    ...
    def calculate_surface_coverage(self, ground_truth_points, reconstructed_points, threshold=0.1):
        """
        Calculate the surface coverage Cs.
        
        Parameters:
        - ground_truth_points: np.ndarray of shape (B, N_gt, 3), ground truth 3D points for B batches.
        - reconstructed_points: np.ndarray of shape (B, N_rec, 3), reconstructed 3D points for B batches.
        - threshold: float, the distance threshold to consider a match.
        
        Returns:
        - Cs: np.ndarray of shape (B,), the surface coverage as a percentage for each batch.
        """
        batch_size = ground_truth_points.shape[0]
        Cs = np.zeros(batch_size)

        for b in range(batch_size):
            # Extract batch points
            gt_points = ground_truth_points[b]
            rec_points = reconstructed_points[b]

            # Total number of ground truth points
            N_gt = gt_points.shape[0]
            
            # Build a k-d tree for the reconstructed points
            tree = KDTree(rec_points)
            
            # For each ground truth point, find the nearest reconstructed point
            distances, _ = tree.query(gt_points)
            
            # Count points with distance less than the threshold
            N_obs = np.sum(distances < threshold)
            
            # Calculate surface coverage for the batch
            Cs[b] = (N_obs / N_gt) * 100
    ...
    def _pre_physics_step(self, actions: torch.Tensor):
        ...
        self._last_xyz = self._camera.data.pos_w.clone()
        ...
        self._xyz = (self._xyz + torch.tensor([0., 0., 1.]).to(self.device)) * torch.tensor([self.cfg.env_size/2.0-5e-1, self.cfg.env_size/2.0-5e-1, self.cfg.env_size/4.0]).to(self.device) # + torch.tensor([0., 0., 2.]).to(self.device)
        #self._xyz = (self._xyz + torch.tensor([0., 0., 1.]).to(self.device)) * torch.tensor([0, self.cfg.env_size/2.0, 0.6*self.cfg.env_size/4.0]).to(self.devicee
    ...
    def _configure_gym_env_spaces(self):
        ...
        self.single_observation_space["policy"]["pose_step"] = gym.spaces.Box(low=-1., high=1., shape=(self.num_observations,))
        # N, 1, H, W
        self.single_observation_space["policy"]["img"] = gym.spaces.Box(low=0., high=1., shape=(self.cfg.img_t, 64, 64))
    ...
    def update_observations(self, env_ids=None):
        ...
        # obv image is 0~1 
        for i in env_ids:
            #self.obv_imgs[i][5] = self.obv_imgs[i][4]
            self.obv_imgs[i][4] = self.obv_imgs[i][3]
            self.obv_imgs[i][3] = self.obv_imgs[i][2]
            self.obv_imgs[i][2] = self.obv_imgs[i][1]
        ...
        points_3d_world_no_shift = points_3d_world.clone()
        for i in env_ids:
            #points_3d_world_no_shift = points_3d_world.clone()
            points_3d_world_no_shift[i,:,0] = points_3d_world[i,:, 0]-self._terrain.env_origins[i][0]
            points_3d_world_no_shift[i,:,1]  = points_3d_world[i,:, 1]-self._terrain.env_origins[i][1]

        #Cs = self.calculate_surface_coverage(self.gt_pointclouds, points_3d_world_no_shift.cpu().numpy(), threshold=0.1)
        #print(Cs)
    ...
    def _get_observations(self) -> dic:
        ...
        #pose_step = current_pose.reshape(self.cfg.num_envs, -1)
        pose_step = self.obv_pose_history.reshape(self.cfg.num_envs, -1)
    ...
    def _get_rewards(self) -> torch.Tensor:
        ...
        self._current_xyz = self._camera.data.pos_w.clone()
        difference = self._current_xyz - self._last_xyz

        # L2 norm across last dimension (D)
        distance = torch.norm(difference, dim=-1).reshape(-1, 1)
        
        #sub_goal_reward = torch.logical_and(self.coverage_ratio_reward>=0.9, self.sub_goal==0)
        sub_goal_mask = (torch.tensor(self.coverage_ratio) > 0.96).float().to(self.device).reshape(-1, 1)
        
        all_mask = ((1 - sub_goal_mask).bool() | col_mask.bool())
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        for env_id in env_ids:
            ...
            #self.gt_pointclouds[env_id] = point_cloud
            # load hollow_occ.npy for coverage ratio computation
