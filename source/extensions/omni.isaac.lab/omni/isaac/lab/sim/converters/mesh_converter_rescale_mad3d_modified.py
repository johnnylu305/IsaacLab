...
def rescale_scene(stage,outfile, scene_prim_root="/World/Scene",max_len=8):
    ...
    #o3d_mesh = o3d_mesh.subdivide_midpoint(number_of_iterations=4)
    #o3d_mesh.translate((0.5, 0.5, 0.5))
    o3d_pcd = o3d_mesh.sample_points_uniformly(number_of_points=100000)
    scaled_points = Vt.Vec3fArray([tuple(vertex) for vertex in np.asarray(o3d_mesh.vertices)])
    scaled_points = Vt.Vec3fArray([tuple(point) for point in np.asarray(o3d_pcd.points)])
    offset=0

    #occ_grid = initialize_occupancy_grid(20,20,20)
    #for v in scaled_points:
    #    vv = (np.array(v)+[5, 5, 0])*2
    #    occ_grid[vv[0], vv[1], vv[2]] = 1
    
    #o3d.visualization.draw([o3d_mesh])
    voxel_size=0.5
    voxel_grid = voxelize_mesh(o3d_mesh, voxel_size)
    print("aaaa", voxel_grid.origin)
    #o3d.visualization.draw([voxel_grid])
    voxel_coordinates = (np.array([voxel_grid.origin + voxel.grid_index*voxel_size + np.array([voxel_size/2, voxel_size/2, voxel_size/2]) for voxel in voxel_grid.get_voxels()]) + np.array([[5, 5, 0]]))*2
    
    #voxel_coordinates = (np.array([voxel_grid.origin + voxel.grid_index*voxel_size for voxel in voxel_grid.get_voxels()]) + np.array([[5, 5, 0]]))*2

    #print(voxel_coordinates.shape)
    voxel_centers = np.floor(voxel_coordinates).astype(int)
    voxel_centers = np.maximum(voxel_centers, 0)
    #import pdb; pdb.set_trace()
    occ_grid = initialize_occupancy_grid(20,20,20)
    for i in range(len(voxel_centers)):
        #print(voxel_centers[i])
        #print(occ_grid.shape)
        #print(min(20-1, voxel_centers[i][0]+10))
        occ_grid[min(20-1, voxel_centers[i][0]), min(20-1, voxel_centers[i][1]), min(20-1, voxel_centers[i][2])]=1
   
    """ 
    occ_grid = initialize_occupancy_grid(20,20,20)
    for v in scaled_points:
        print(v)
        print(np.array(v)+[5, 5, 0])
        vv = np.floor((np.array(v)+[5, 5, 0])*2).astype(int)
        occ_grid[min(vv[0], 19), min(vv[1], 19), min(vv[2], 19)] = 1
    """

    # Assuming scaled_points is a NumPy array
    scaled_points_array = np.asarray(scaled_points)  # Ensure scaled_points is a NumPy array
    # Translate and scale points
    translated_scaled_points = np.floor((scaled_points_array + [5, 5, 0]) * 2).astype(int)
    # Clamp the values to ensure they don't exceed the grid dimensions (19 here)
    clamped_points = np.minimum(translated_scaled_points, 19)
    # Update the occupancy grid using broadcasting
    occ_grid[clamped_points[:, 0], clamped_points[:, 1], clamped_points[:, 2]] = 1
