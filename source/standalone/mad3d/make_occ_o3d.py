...
def rescale_scene(stage, scene_path, scene_prim_root="/World/Scene"):
    all_points = []
    meshes = []
    ...
    mesh_prim = get_prim_at_path(prim_path="/World/Scene")
    ...
    xform.AddTransformOp().Set(combined_transform)
        ...
        # Step 2: Convert Gf.Quatd to a NumPy array for easier manipulation
    w, x, y, z = cumulative_rotation.GetReal(), *cumulative_rotation.GetImaginary()
    w = -w

    # Step 3: Compute the 3x3 rotation matrix from the quaternion
    # Using the standard formula for a rotation matrix from a quaternion
    ...
    # Step 4: Embed the 3x3 matrix in a Gf.Matrix4d
    rotation_matrix_4x4 = Gf.Matrix4d(1.0)  # Start with an identity matrix
    for i in range(3):
        for j in range(3):
            rotation_matrix_4x4[i, j] = rotation_matrix_3x3[i, j]
    # texture_path = r'/home/hat/Documents/usd_objaverse_test/glbs/000-000/00f665c4abc04d48bdcb430131bc5b73/textures/00f665c4abc04d48bdcb430131bc5b73_texture0.png'
    # # Load the texture using PIL
    # texture_image = Image.open(texture_path)
    # texture_image = texture_image.convert("RGB")  # Ensure the image is RGB

    # # Convert texture to a NumPy array and ensure it is of type uint8
    # texture_image_np = np.asarray(texture_image, dtype=np.uint8)

    # # Create an Open3D Image from the texture NumPy array
    # o3d_texture = o3d.geometry.Image(texture_image_np)
    ...
    # Get the original /World/Scene/Looks folder
    #looks_path = "/World/Scene/Looks"
    #looks_prim = stage.GetPrimAtPath(looks_path)

    # for child in original_prim.GetChildren():
    #     if '/Looks' not in child:
    #         original_prim_path = 
    # import pdb; pdb.set_trace()

    # Define the path for the new mesh prim
    #new_mesh_prim_path = "/World/Scene/geometry"  # Reuse the original path if desired

    # Unpack merged_mesh into points, face vertex counts, and face vertex indices
    #merged_points, merged_face_vertex_counts, merged_face_vertex_indices = merged_mesh

    stage.RemovePrim(original_prim_path)


    return mapped_axis, occ_grid, centroid, cumulative_rotation, stage

def apply_texture_scaling(usd_mesh, scale_factor):
    # Scale UV coordinates if texture coordinates (primvar:st) exist
    uv_attr = usd_mesh.GetPrim().GetAttribute("primvars:st")
    if uv_attr.IsValid():
        # Get and scale UV coordinates
        uv_coords = np.array(uv_attr.Get())
        scaled_uv_coords = uv_coords * scale_factor
        uv_attr.Set(scaled_uv_coords)
...
def convert_o3d_mesh_to_numpy(o3d_mesh):
    """
    Converts an Open3D mesh to NumPy arrays for vertices, face vertex counts, and face vertex indices.
    
    Parameters:
    o3d_mesh (o3d.geometry.TriangleMesh): The Open3D mesh object to convert
    
    Returns:
    tuple: (merged_points, merged_face_vertex_counts, merged_face_vertex_indices)
           - merged_points: Nx3 array of vertex positions
           - merged_face_vertex_counts: Array where each entry is 3, indicating triangles
           - merged_face_vertex_indices: Flat array of vertex indices forming each triangle
    """
    # Extract vertex positions as an Nx3 NumPy array
    merged_points = np.asarray(o3d_mesh.vertices)

    # Each face is a triangle, so face vertex count will be 3 for each face
    merged_face_vertex_counts = np.full(len(o3d_mesh.triangles), 3, dtype=int)

    # Extract triangle indices and flatten into a single array
    merged_face_vertex_indices = np.asarray(o3d_mesh.triangles).flatten()

    return merged_points, merged_face_vertex_counts, merged_face_vertex_indices


def run_simulator(sim, output, stage,scene_path):
    """Run the simulator."""
    ...
    #o3d_mesh.translate((-centroid[0], -centroid[1], -centroid[2]))
    #o3d_mesh.scale(scale_factor, center=(0, 0, 0))
    # Define the angle in radians (-90 degrees along the X-axis)
    #angle = np.radians(90)
    #axis = np.array([1, 0, 0])  # X-axis

    # Generate a 3x3 rotation matrix from the axis and angle
    # rotation_matrix = o3d_mesh.get_rotation_matrix_from_axis_angle(axis * angle)
...
def main():
    scenes_path = sorted(glob.glob(os.path.join(args_cli.input, '**', '*[!_rescaled][!_non_metric].usd'), recursive=True))
    ...
    for i, scene_path in enumerate(scenes_path):
        ...
        if args_cli.rescale:
            up_axis, save_o3d_mesh,centroid, cumulative_rotation, stage= rescale_scene(stage, scene_path)
        #import pdb; pdb.set_trace()
        ...
        run_simulator(world, output, stage, scene_path)
