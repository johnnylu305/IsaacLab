...
from sb3_on_policy_cus import OnPolicyAlgorithmCus

from sb3_policy_cus  import MultiInputActorCriticPolicyCus

SelfPPO = TypeVar("SelfPPO", bound="PPO")


class PPO_Cus(OnPolicyAlgorithmCus):
    """
    ...
    policy_aliases: ClassVar[Dict[str, Type[BasePolicy]]] = {
        ...
        "MultiInputPolicyCus": MultiInputActorCriticPolicyCus,
    }
    ...
    def train(self) -> None:
        ...
        for epoch in range(self.n_epochs):
            ...
            for rollout_data in self.rollout_buffer.get(self.batch_size):
                # we have remove xyz from rollout
                ...
                # rollout is old data 
                """
                xyz = rollout_data.xyzs
                print("xyz", xyz.shape)
                cov = rollout_data.covs
                print("cov", cov.shape)
                """
                aux_cov = rollout_data.observations["aux_cov"]
                #print("xyz", xyz.shape)
                #print("cov", cov.shape)
                #print("obv cov", aux_cov.shape)

                aux_cov_loss = th.mean((cov - aux_cov.detach())**2)

                aux_cov_losses.append(aux_cov_loss.detach().cpu().numpy())
                #print(off.shape, gt_off.shape)
                aux_off_loss = th.mean((off - gt_off)**2)
