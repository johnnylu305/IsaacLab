...
import time
import warnings
from typing import Any, Dict, Generator, List, NamedTuple, Optional, Tuple, Type, TypeVar, Union
...
class DictRolloutBufferSamplesCus(NamedTuple):
...
class DictRolloutBufferCus(RolloutBuffer):
    """
    ...
    def reset(self) -> None:
        ...
        self.generator_ready = False
        # customize
        self.xyzs = np.zeros((self.buffer_size, self.n_envs, 3), dtype=np.float32)
        self.covs = np.zeros((self.buffer_size, self.n_envs, 1), dtype=np.float32)
    ...
    def add(  # type: ignore[override]
        ...
        # Reshape to handle multi-dim and discrete action spaces, see GH #970 #1392
        xyz = xyz.reshape((self.n_envs, 3))
        cov = cov.reshape((self.n_envs, 1))

        self.actions[self.pos] = np.array(action)
        self.xyzs[self.pos] = np.array(xyz)
        self.covs[self.pos] = np.array(cov)
    ...
    def get(  # type: ignore[override]
    ...
    ) -> Generator[DictRolloutBufferSamplesCus, None, None]:
        assert self.full, ""
        ...
        if not self.generator_ready:
            ...
            _tensor_names = ["actions", "values", "log_probs", "advantages", "returns", "xyzs", "covs"]
    ...
    ) -> DictRolloutBufferSamplesCus:
        return DictRolloutBufferSamplesCus(
            ...
            xyzs=self.to_torch(self.xyzs[batch_inds]),
            covs=self.to_torch(self.covs[batch_inds]),
        )


class OnPolicyAlgorithmCus(BaseAlgorithm):
    """
    ...
    def _setup_model(self) -> None:
        ...
        if self.rollout_buffer_class is None:
            if isinstance(self.observation_space, spaces.Dict):
                self.rollout_buffer_class = DictRolloutBufferCus
            else:
    ...
    def collect_rollouts(
        ...
        while n_steps < n_rollout_steps:
            ...
            # we need cov after taking action
            self._last_obs["aux_cov"] = new_obs["aux_cov"]
