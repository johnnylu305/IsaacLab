...
def merge_images_to_video(folder_path, output_video, fps=30):
    ...
    files = [f for f in os.listdir(folder_path) if f.startswith("step") and f.endswith(".png")]
...
if __name__ == "__main__":
    ...
    folder_path = "/projects/MAD3D/Zhuoli/IsaacLab/output"
    for folder in os.listdir(folder_path):
        ...
        if os.path.isdir(os.path.join(folder_path, folder)):
            ...
            #print(folder)
            output_video = os.path.join(folder_path,f"{folder}_video.mp4")  # Replace with the desired output video name
