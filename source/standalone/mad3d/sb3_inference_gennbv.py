...
def setup_scene(world, scene_path, index, scene_prim_root="/World/Scene"):
    ...
    # define the property of the stagescene_prim = XFormPrim(prim_path=scene_prim_root, translation=TRANS)
    scene_prim = XFormPrim(prim_path=scene_prim_root, translation=TRANS)
    # activate the stage
...
def run_simulator(sim, scene_entities, agent, hollow_occ_path, gt_pcd_path, coverage_ratio_rec, cd_rec, acc_rec, scene_id, end, dataset_name):
    ...
    for index in range(NUM_STEPS):
        ...
        #_xyz = (_xyz + torch.tensor([0., 0., 1.]).to(DEVICE)) * torch.tensor([env_size/2.0-5e-1, env_size/2.0-5e-1, env_size/4.0]).to(DEVICE) # + torch.tensor([0., 0., 2.]).to(self.device)
        ...
        #_pitch = (actions[:,4]+1/5.)/2.*torch.pi*5/6.*-1


        # save data
        #x, y, z = new_positions[0, :3]
        #_yaw = _yaw.squeeze()
        x, y, z = pose[0, :3]
        _yaw, _pitch = pose[0, 3:]
        _pitch = _pitch.squeeze()
